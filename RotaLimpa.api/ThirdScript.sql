...
SET @defaultSchema = SCHEMA_NAME();
DECLARE @description AS sql_variant;
SET @description = N'placa do veiculo';
EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'Frota', 'COLUMN', N'P_Veiculo';
...
CREATE TABLE [Colaborador] (
    ...
    CONSTRAINT [FK_Colaborador_Empresa_IdEmpresa] FOREIGN KEY ([IdEmpresa]) REFERENCES [Empresa] ([Id]) ON DELETE CASCADE
);
DECLARE @defaultSchema AS sysname;
SET @defaultSchema = SCHEMA_NAME();
DECLARE @description AS sql_variant;
...
CREATE TABLE [HistoLonginMotorista] (
    [Historico_Login] int NOT NULL IDENTITY,
...
CREATE TABLE [Setor] (
    ...
    CONSTRAINT [FK_Setor_Empresa_IdEmpresa] FOREIGN KEY ([IdEmpresa]) REFERENCES [Empresa] ([Id]) ON DELETE NO ACTION
);
GO
...
CREATE TABLE [Rota] (
    ...
    CONSTRAINT [FK_Rota_Setor_IdSetor] FOREIGN KEY ([IdSetor]) REFERENCES [Setor] ([Id]) ON DELETE NO ACTION
);
DECLARE @defaultSchema AS sysname;
SET @defaultSchema = SCHEMA_NAME();
DECLARE @description AS sql_variant;
...
CREATE TABLE [RelatorioFinal] (
    ...
    CONSTRAINT [FK_RelatorioFinal_Setor_IdSetor] FOREIGN KEY ([IdSetor]) REFERENCES [Setor] ([Id]) ON DELETE NO ACTION
);
GO
...
CREATE INDEX [IX_Colaborador_IdEmpresa] ON [Colaborador] ([IdEmpresa]);
GO
...
VALUES (N'20231122234930_InicialMigration', N'7.0.10');
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231123002732_ColaboradorMigration', N'7.0.10');
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231123004503_ModelMigration', N'7.0.10');
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231123004525_DTOMigration', N'7.0.10');
GO
...
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Motorista]') AND [c].[name] = N'Login');
IF @var0 IS NOT NULL EXEC(N'ALTER TABLE [Motorista] DROP CONSTRAINT [' + @var0 + '];');
ALTER TABLE [Motorista] ALTER COLUMN [Login] nvarchar(7) NULL;
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Colaborador]') AND [c].[name] = N'Login');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [Colaborador] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [Colaborador] ALTER COLUMN [Login] nvarchar(7) NULL;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231123004854_LoginMigration', N'7.0.10');
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231123104657_FinalMigration', N'7.0.10');
GO
...
ALTER TABLE [RelatorioFinal] DROP CONSTRAINT [FK_RelatorioFinal_Ocorrencia_IdOcorrencia];
GO

EXEC sp_rename N'[RelatorioFinal].[IdOcorrencia]', N'IdTrajeto', N'COLUMN';
GO

EXEC sp_rename N'[RelatorioFinal].[IX_RelatorioFinal_IdOcorrencia]', N'IX_RelatorioFinal_IdTrajeto', N'INDEX';
GO

ALTER TABLE [RelatorioFinal] ADD [OcorrenciaId] int NULL;
GO

ALTER TABLE [Ocorrencia] ADD [Id_Cep] int NOT NULL DEFAULT 0;
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Frota]') AND [c].[name] = N'P_Veiculo');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [Frota] DROP CONSTRAINT [' + @var2 + '];');
ALTER TABLE [Frota] ALTER COLUMN [P_Veiculo] nvarchar(7) NOT NULL;
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Empresa]') AND [c].[name] = N'St_empresa');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Empresa] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [Empresa] ALTER COLUMN [St_empresa] nvarchar(max) NOT NULL;
GO

CREATE INDEX [IX_RelatorioFinal_OcorrenciaId] ON [RelatorioFinal] ([OcorrenciaId]);
GO

CREATE INDEX [IX_Ocorrencia_Id_Cep] ON [Ocorrencia] ([Id_Cep]);
GO

CREATE UNIQUE INDEX [IX_Motorista_Id] ON [Motorista] ([Id]);
GO

ALTER TABLE [Ocorrencia] ADD CONSTRAINT [FK_Ocorrencia_CEP_Id_Cep] FOREIGN KEY ([Id_Cep]) REFERENCES [CEP] ([Id]) ON DELETE CASCADE;
GO

ALTER TABLE [RelatorioFinal] ADD CONSTRAINT [FK_RelatorioFinal_Ocorrencia_OcorrenciaId] FOREIGN KEY ([OcorrenciaId]) REFERENCES [Ocorrencia] ([Id]);
GO

ALTER TABLE [RelatorioFinal] ADD CONSTRAINT [FK_RelatorioFinal_Trajeto_IdTrajeto] FOREIGN KEY ([IdTrajeto]) REFERENCES [Trajeto] ([Id]) ON DELETE CASCADE;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20231201000717_213011', N'7.0.10');
GO
